...
class CreateTaskPage extends Component {
  ...
  handleSubmitTask = async () => {
    const { tasksStore, routerStore } = this.props as any;
    const { title, description } = this.state as any;
    ...
    return (
      <FormWrapper>
        <FormContainer>
          ...
          { (this.state as any).errorMessage && <ErrorMessage message={(this.state as any).errorMessage} />}
